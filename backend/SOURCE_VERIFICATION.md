...
| PFZ WFS | [GeoJSON](https://incois.gov.in/geoserver/PFZ_Automation/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=PFZ_Automation:pfzlines&outputFormat=application/json) | MultiLineString coordinates in longitude/latitude. Validity is separately matched to the advisory table. A date-only "Valid upto" value is treated as inclusive through that calendar day in Asia/Kolkata because no exact expiry hour is published. |
