...
class OceanAgent(BaseAgent):
    ...
    async def _fetch_at_location(self, loc: Location, req_time: datetime | None, tag: str = 'origin') -> tuple[dict, list[EvidenceItem], list[str]]:
        conditions = await self.service.conditions('ocean', loc, req_time)
        evidence = []
        ...
        if action == 'conditions_for_location':
            if not input_data.location:
                return AgentOutput(
                    agent=self.name,
                    status='failed',
                    errors=['Location is required for ocean conditions'],
                    confidence=0.0,
                )
            ...
            return AgentOutput(
                ...
                evidence=evidence,
                warnings=warnings,
                confidence=0.94 if evidence else 0.4,
            )
            ...
            return AgentOutput(
                ...
                data={'candidate_ocean': cand_data},
