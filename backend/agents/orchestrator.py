...
class MarineOrchestrator:
    ...
    async def chat(
        ...
        latest_published_pfz_candidates = []
        if pfz_out:
            latest_published_pfz_candidates = [
                candidate for candidate in pfz_out.data.get('candidates', [])
                if candidate.get('reference_only')
            ]
        latest_published_pfz = (
            latest_published_pfz_candidates[0] if latest_published_pfz_candidates else None
        )
        ...
        analysis_summary = {
            ...
            'latest_published_pfz_candidates': latest_published_pfz_candidates,
        }
