...
async (page) => {
  ...
  const greetingResponse = page.waitForResponse((response) => response.url().includes('/api/v1/chat'));
  await page.locator('#orca-query').fill('hello');
  await page.getByRole('button', { name: 'Send question', exact: true }).click();
  const greeting = await (await greetingResponse).json();
  assert(greeting.intent === 'general_conversation' && greeting.answer, 'Greeting entered marine analysis');
  await page.locator('.chat-thinking').waitFor({ state: 'hidden', timeout: 45000 });
  assert(await page.locator('.chat-answer').last().locator('p').textContent() === greeting.answer, 'Greeting was not rendered');
  assert(!(await page.locator('.chat-answer__meta').last().textContent()).includes('confidence'), 'Conversation shows marine confidence');
  const smallTalkResponse = page.waitForResponse((response) => response.url().includes('/api/v1/chat'));
  await page.locator('#orca-follow-up').fill('How are you?');
  await page.getByRole('button', { name: 'Send follow-up', exact: true }).click();
  const smallTalk = await (await smallTalkResponse).json();
  assert(smallTalk.intent === 'general_conversation' && smallTalk.session_id === greeting.session_id, 'Small talk lost conversation context');
  await page.locator('.chat-thinking').waitFor({ state: 'hidden', timeout: 45000 });
  await page.locator('.orca-nav__rail a[href="#home"]').click();
